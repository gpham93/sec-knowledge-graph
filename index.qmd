...
<div class="mb-3">
  ...
  <span class="badge-custom badge-exec">Executive</span>
  <span class="badge-custom badge-ind">Industry</span>
  <span class="badge-custom badge-risk">Risk Factor</span>
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const colorMap = {
    'Corporation': { bg: '#e5c158', border: '#d4af37', size: 28 },
    'Executive': { bg: '#9d4edd', border: '#7b2cbf', size: 20 },
    'Industry': { bg: '#2ec4b6', border: '#0f9f90', size: 22 },
    'RiskFactor': { bg: '#e63946', border: '#d62828', size: 18 },
    'Subsidiary': { bg: '#00b4d8', border: '#0077b6', size: 16 }
  };
  ...
  fetch('data_graph.json')
    ...
    .then(data => {
      ...
      const styledNodes = data.nodes.map(node => {
        const style = colorMap[node.group] || colorMap['Subsidiary'];
        return {
          ...node, // Preserve CIK, SIC, executive title, risk category, jurisdiction, etc.
          shape: 'dot',
          size: style.size,
          color: {
            background: style.bg,
          ...
          font: {
            color: textColor,
            size: node.group === 'Corporation' ? 15 : 12,
      ...
      network.on("click", function(params) {
        if (params.nodes.length > 0) {
          ...
          if (nodeData.group === 'Corporation') {
            ...
            `;
          } else if (nodeData.group === 'Executive') {
            detailsHtml += `
              <strong>Executive Title:</strong> ${nodeData.title || 'Executive Officer'}
            `;
          } else if (nodeData.group === 'Industry') {
            detailsHtml += `
              <strong>SIC Classification Code:</strong> ${nodeData.sicCode || 'Unknown'}
            `;
          } else if (nodeData.group === 'RiskFactor') {
            detailsHtml += `
              <strong>Item 1A Risk Profile:</strong> ${nodeData.riskCategory || 'Operational Risk'}
            `;
          } else {
            detailsHtml += `
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const FINANCIAL_TERM_MAP = {
    ...
    "subsidiaries": ["exhibit 21", "subsidiaries of the registrant", "owned entities"]
  };

  function expandFinancialTerms(queryText) {
    let expanded = queryText;
    const lower = queryText.toLowerCase();
    for (const [key, synonyms] of Object.entries(FINANCIAL_TERM_MAP)) {
  ...
  function queryGemini(userMessage) {
    ...
    const expandedQuery = expandFinancialTerms(userMessage);

    const prompt = `
You are an expert corporate intelligence assistant with access to a dual-pass hybrid Knowledge Graph facts database derived from SEC EDGAR 10-K filings.

${graphDataText}

User Query: "${userMessage}"
Expanded Domain Search Context: "${expandedQuery}"

Rules:
1. Ground your answer strictly in the facts database above. If structural entities (subsidiaries, executives) are requested, list them clearly.
2. If narrative topics (risks, supply chain, lawsuits) are requested, use the expanded SEC 10-K terms to summarize key risks.
3. Be concise and cite parent-subsidiary relationships or metadata explicitly.
`;
