...
::: {.graph-card}
### Graph RAG AI Copilot
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <span class="badge-custom badge-corp">Grounded in SEC Graph</span>
  </div>
  <button id="toggle-key-btn" class="btn btn-sm btn-outline-secondary" style="font-family: 'Outfit', sans-serif;">⚙️ Configure API Key</button>
</div>

<div id="key-panel" class="api-key-panel d-none">
  ...
  <small class="text-muted">No backend is used. Your key is sent directly to Google's Gemini API endpoint in your browser.</small>
</div>

<div id="chat-log" class="chat-log">
  <div class="chat-message bot">
    Hello! I am your SEC Graph RAG assistant. Set your Gemini API key above to start querying, or try one of the suggestion questions below:
  </div>
</div>

<div class="chat-chips">
  <div class="chip" onclick="sendSuggestion('What is Morgan Stanley\'s CIK and state of incorporation?')">Morgan Stanley Info</div>
  <div class="chip" onclick="sendSuggestion('Which JPMorgan Chase subsidiaries are located in Germany?')">JPM Germany Subs</div>
  <div class="chip" onclick="sendSuggestion('Are there any duplicate or clustered subsidiaries?')">Deduplication Info</div>
  <div class="chip" onclick="sendSuggestion('What is Goldman Sachs\' business address?')">GS Address</div>
</div>

<div class="input-group">
  <input type="text" id="chat-input" class="form-control" placeholder="Ask about CIKs, SIC codes, addresses, or subsidiaries...">
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  fetch('data_graph.json')
    ...
    .then(data => {
      ...
      corps.forEach(c => {
        context += `- ${c.label} (URI: ${c.id}) has CIK ${c.cik}, SIC code ${c.sic} (${c.sicDescription}), state of incorporation ${c.stateOfIncorporation}, address: ${c.businessAddress.replace(/\\n/g, ', ')}.\n`;
      });
  ...
  function queryGemini(userMessage) {
    ...
    if (!apiKey) {
      appendMessage('bot', '⚠️ Please configure your Gemini API Key using the gear settings button above to enable Graph RAG chatbot queries.');
      return;
    }
...
`;
  ...
  // Bind to window to allow chip triggers
